...
const MatrixRain: React.FC<{ onExit: () => void }> = ({ onExit }) => {
  ...
  useEffect(() => {
    ...
    const rainDrops = Array.from({ length: Math.ceil(columns) }).fill(1) as number[];
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black cursor-pointer" onClick={onExit}>
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-zinc-900 border border-zinc-700 p-6 rounded-xl text-center backdrop-blur-sm">
            <h1 className="text-3xl font-bold text-green-400 mb-2 tracking-widest uppercase glitch-text">System Hacked</h1>
            <p className="text-zinc-400 font-mono text-sm blinking-cursor">Press any key or click to return...</p>
        </div>
    </div>
  );
};
...
const ExternalLinkLoader: React.FC<ExternalLinkLoaderProps> = ({ platform, url, colorClass, onSuccess }) => {
  ...
  return (
    <div className={`space-y-2 font-mono ${stage === 'done' ? 'opacity-50' : ''}`}>
        <div className="flex items-center gap-2">
            <span className="text-zinc-500 text-xs">Target:</span>
            <span className={`font-bold transition-all duration-300 ${stage === 'success' ? 'text-emerald-400 tracking-widest' : colorClass}`}>
                {platform}
            </span>
            {stage === 'success' && (
                <span className="text-emerald-500 text-xs font-bold animate-pulse ml-2">[ SECURE ]</span>
            )}
        </div>

        {/* Progress Bar Container */}
        <div className={`w-full max-w-[240px] h-1.5 bg-zinc-800 rounded overflow-hidden border transition-colors duration-300 ${stage === 'success' ? 'border-emerald-500/50' : 'border-zinc-700'}`}>
            {/* The Bar */}
            <div 
                className={`h-full ${colorClass.replace('text-', 'bg-')} transition-all duration-300 ${stage === 'success' ? '!bg-emerald-500' : 'animate-progress'}`} 
                style={{ width: stage === 'success' || stage === 'done' ? '100%' : undefined }}
            />
        </div>
        ...
        <div className="flex flex-col text-[10px] text-zinc-500 font-mono h-8 justify-end">
            ...
            {stage === 'success' && (
                <span className="text-emerald-400 font-bold animate-pulse">> UPLINK ESTABLISHED.</span>
            )}
            {stage === 'done' && (
...
const Terminal: React.FC = () => {
  const [history, setHistory] = useState<CommandHistory[]>([
    { type: 'output', content: <span className="text-zinc-400">Welcome. Type <span className="text-zinc-200 font-bold">help</span> to view commands.</span> }
  ]);
  ...
  const availableCommands = [
    ...
    'contact', 'email', 'clear', 'github', 'linkedin', 'leetcode', 'twitter',
    'matrix', 'sudo'
  ];
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setInput(val);
    if (val.trim()) {
      const matches = availableCommands.filter(cmd => cmd.startsWith(val.toLowerCase()));
  ...
  const executeCommand = (cmd: string) => {
    ...
    const newHistory: CommandHistory[] = [...history, { type: 'command', content: cmd }];
    let output: React.ReactNode = '';

    if (isSudo) {
         output = <span className="text-red-400 font-bold bg-red-400/10 px-2 py-1 rounded">ACCESS DENIED. This incident will be reported.</span>;
    } else {
        switch (cleanCmd) {
        case 'help':
            output = (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-zinc-300">
                {availableCommands.map(c => <span key={c}>{c}</span>)}
            </div>
            );
            break;
        case 'about':
            ...
            <div className="space-y-2">
                <p>User: <span className="text-zinc-200 font-bold">{PERSONAL_INFO.name}</span></p>
                <p className="text-zinc-400">{PERSONAL_INFO.bio}</p>
                <p className="text-zinc-500 text-xs">Location: {PERSONAL_INFO.location}</p>
            </div>
            );
            break;
        case 'skills':
            ...
            <div className="space-y-2">
                {SKILLS.map(s => (
                <div key={s.category}>
                    <span className="text-zinc-400 font-bold text-xs uppercase tracking-wide">{s.category}:</span>{' '}
                ...
                <div className="space-y-3">
                    {PROJECTS.map(p => (
                        <div key={p.id}>
                            ...
                            <div className="text-xs text-zinc-500">{p.tech.join(', ')}</div>
                        </div>
                    ))}
                </div>
            )
        ...
        case 'contact':
            ...
            <div className="space-y-1">
                <p>Email: <a href={`mailto:${PERSONAL_INFO.email}`} className="text-zinc-300 hover:underline">{PERSONAL_INFO.email}</a></p>
                <p>Phone: <span className="text-zinc-400">{PERSONAL_INFO.phone}</span></p>
            </div>
            );
            break;
        case 'email':
            output = <ExternalLinkLoader platform="Gmail" url={`https://mail.google.com/mail/?view=cm&fs=1&to=${PERSONAL_INFO.email}`} colorClass="text-red-400" onSuccess={triggerSuccessFlash} />;
            break;
        case 'github':
            output = <ExternalLinkLoader platform="GitHub" url={PERSONAL_INFO.github} colorClass="text-zinc-300" onSuccess={triggerSuccessFlash} />;
            break;
        case 'linkedin':
            output = <ExternalLinkLoader platform="LinkedIn" url={PERSONAL_INFO.linkedin} colorClass="text-blue-400" onSuccess={triggerSuccessFlash} />;
            break;
        case 'leetcode':
            output = <ExternalLinkLoader platform="LeetCode" url={PERSONAL_INFO.leetcode} colorClass="text-amber-400" onSuccess={triggerSuccessFlash} />;
            break;
        case 'twitter':
             output = <ExternalLinkLoader platform="Twitter" url={PERSONAL_INFO.twitter} colorClass="text-sky-400" onSuccess={triggerSuccessFlash} />;
             break;
        case 'matrix':
        ...
        output = (
            <div className="flex flex-col gap-2">
                ...
                <div className="text-zinc-500 text-xs flex items-center gap-2 pt-2 border-t border-zinc-800 mt-1 w-full sm:w-max">
                    ...
                    <span>Try</span>
                    <button 
                        onClick={() => { setInput(nextCmd); inputRef.current?.focus(); }}
  ...
  return (
    <>
      ...
      <div className={`w-full max-w-2xl mx-auto my-12 rounded-xl bg-[#0f0f11] border overflow-hidden shadow-xl font-mono text-sm transition-all duration-100 
